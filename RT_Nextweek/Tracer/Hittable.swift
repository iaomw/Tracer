...
class NormalFlipped: Hittable {
    ...
    func hitTest(ray: Ray, t_min: Float, t_max: Float) -> HitRecord? {
        if let r = hittable.hitTest(ray: ray, t_min: t_min, t_max: t_max) {
            return HitRecord(t: r.t, p: r.p, n: -r.n, m: r.m, u: r.u, v: r.v)
        }
        return nil
    }
...
class Translate: Hittable {
    ...
    func hitTest(ray: Ray, t_min: Float, t_max: Float) -> HitRecord? {
        ...
        if let r = hittable.hitTest(ray: rayOffsetted, t_min: t_min, t_max: t_max) {
            return HitRecord(t: r.t, p: r.p+offset, n: r.n, m: r.m, u: r.u, v: r.v)
        }
        return nil
    }
