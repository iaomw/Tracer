...
typealias SceneComplex = (HittableList, ((Float) -> Camera), ((Ray, Hittable, Int) -> Vec3))
...
class Render {
    ...
    lazy var randomScene: (()->SceneComplex?) = {
        ...
        return (HittableList(list: [treeBVH]), self.camera0, self.color0)
    }
    ...
    lazy var twoSpheres: (()->SceneComplex?) = {
        ...
        return (HittableList(list: list), self.camera0, self.color0)
    }
    ...
    lazy var cornellBox: (()->SceneComplex?) = {
        ...
        let list: [Hittable] = [
            ...
            Rect(.x, 213, 343, .z, 227, 332, 554, light),
    ...
    func color0(ray: Ray, world: Hittable, depth: Int) -> Vec3 {
        ...
        if depth < 50 {
            if let (scattered, attenuation) = hitRecord.m.scatter(ray: ray, hitRecord: hitRecord) {
                return attenuation*color0(ray: scattered, world: world, depth: depth+1)
            }
        }
    ...
    func color1(ray: Ray, world: Hittable, depth: Int) -> Vec3 {
        guard let hitRecord = world.hitTest(ray: ray, t_min: 0.001, t_max: Float.greatestFiniteMagnitude) else {
            return Vec3()
        }
        
        let emitted = hitRecord.m.emitted(u: hitRecord.u, v: hitRecord.v, p: hitRecord.p)
        if depth < 50 {
            if let (scattered, attenuation) = hitRecord.m.scatter(ray: ray, hitRecord: hitRecord) {
                return emitted + attenuation*color1(ray: scattered, world: world, depth: depth+1)
            }
        }
            ...
            for value in 0..<upper {
                ...
                for i in 0..<nx {
                    ...
                    for _ in 0..<ns {
                        ...
                        col += colorFunction(ray, world, 0)
                    }
                    col = col/Float(ns)
                    
                    var nnn = Int32(3)
                    let raw = [col.x, col.y, col.z]
                    var root = [col.x, col.y, col.z]
                    vvsqrtf(&root, raw, &nnn)
                    let fff = simd_float3(root[0], root[1], root[2])*255.99
